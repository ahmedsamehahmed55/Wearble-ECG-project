...
#define M 			5 // High pass filter
#define N 			30 // Low pass filter
#define winSize			250 
...
void loop() {        
  currentMicros = micros(); // micros() is a global function 

  // iterate if it's time for a new data point (according to PERIOD). Ensures sampling rate of 250 Hz.
  if (currentMicros - previousMicros >= PERIOD) { // it will not run the ECG reading before the 4000us
    // save the last time you blinked the LED
    previousMicros = currentMicros;

    // see if it's time to turn LED off if it is on
  ...
  */
    ...
    // Enter the function if it the leads are on only.
    if(leads_are_on){           
...
float ecg_buff[M + 1] = {0}; // Initialize values of the array to be zeros
int ecg_buff_WR_idx = 0;
int ecg_buff_RD_idx = 0;

// circular buffer for input ecg signal
// we need to keep a history of N+1 samples for LP filter
float hp_buff[N + 1] = {0}; // Initialize values of the array to be zeros
...
boolean detect(float new_ecg_pt) {
	...
	if(number_iter < N){
		...
		lp_sum -= hp_buff[tmp] * hp_buff[tmp];  //Squaring
